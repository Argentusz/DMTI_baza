...
func Addition1(x Natural) Natural {
	...
	if x.Digits[x.Older] == 9 { // Если последний разряд равен 9, то при прибавлении единицы, он становится 0
		x.Digits[x.Older] = 0
		for i = x.Older - 1; i >= 0; i-- { // Прибавляем единицу к первому разряду, не равному 9(9 заменяем на 0)
			if x.Digits[i] < 9 {
				x.Digits[i] += 1
				return x
			} else {
	...
	x.Digits[x.Older] = 0 // Присваиваем 0 только последнему разряду, так как остальным он уже присвоен в цикле
	return x
}

//Хвостовский Добавление 1 к натуральному числу
func Addiction1(x Natural) Natural {
	var i uint32
	if x.Digits[x.Older] == 9 { // Если последний разряд равен 9, то при прибавлении единицы, он становится 0
		x.Digits[x.Older] = 0
		for i = x.Older - 1; i >= 0; i-- { // Прибавляем единицу к первому разряду, не равному 9(9 заменяем на 0)
